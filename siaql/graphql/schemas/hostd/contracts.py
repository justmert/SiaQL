...
class ContractQueries(HostdBaseResolver):
    ...
    async def contract_integrity(self, info: Info, id: FileContractID) -> Optional[IntegrityCheckResult]:
        ...
        return await self.handle_api_call(info, "get_contract_integrity", id=id)


@strawberry.type
class ContractMutations(HostdBaseResolver):
    @strawberry.mutation
    async def check_contract_integrity(self, info: Info, id: FileContractID) -> bool:
        """Start integrity check for a contract"""
        await self.handle_api_call(info, "put_contract_integrity", id=id)
        return True

    @strawberry.mutation
    async def delete_contract_integrity(self, info: Info, id: FileContractID) -> bool:
        """Delete integrity check result for a contract"""
        await self.handle_api_call(info, "delete_contract_integrity", id=id)
        return True
