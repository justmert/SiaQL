...
class ConsensusQueries(WalletdBaseResolver):
    ...
    async def consensus_network(self, info: Info) -> Network:
        ...
        return await WalletdBaseResolver.handle_api_call(info, "get_consensus_network")

    @strawberry.field
    async def consensus_tip(self, info: Info) -> ChainIndex:
        """Get the current consensus tip"""
        return await WalletdBaseResolver.handle_api_call(info, "get_consensus_tip")

    @strawberry.field
    async def consensus_tip_state(self, info: Info) -> ConsensusState:
        """Get the current consensus tip state"""
        return await WalletdBaseResolver.handle_api_call(info, "get_consensus_tip_state")

    @strawberry.field
    async def consensus_index(self, info: Info, height: int) -> ChainIndex:
        """Get consensus index at specified height"""
        return await WalletdBaseResolver.handle_api_call(info, "get_consensus_index", height=height)

    @strawberry.field
    async def consensus_updates(
        self, info: Info, index: ChainIndex.Input, limit: int = 10
    ) -> List[Union[ApplyUpdate, RevertUpdate]]:
        """Get consensus updates since specified index"""
        return await WalletdBaseResolver.handle_api_call(info, "get_consensus_updates", index=index, limit=limit)
